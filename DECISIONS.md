...
**Update (post-Phase 2, July 2026):** A third faithfulness measurement came back at 0.8833 (down from 0.9056, down from the original 0.9626) — still passing the 0.75 CI gate. Ran a per-question diff against the Day 15 baseline (`eval_results_per_question.csv`, both runs) rather than treating the aggregate number at face value.

**Finding:** 26 of 30 questions scored bit-identical faithfulness across both runs — confirms retrieval, embeddings, and reranking are fully deterministic and reproducible; rules out corpus drift, re-embedding, or a silent model swap as the cause. Only 4 questions moved, and not in one direction: three dropped (Attention Is All You Need's attention-distribution question, Engineering the RAG Stack's reasoning-misalignment question, and the Claude Opus 4.6 nonexistent-repo question — each by 0.167–0.500), one rose by the same margin (the Gemma 3 27B MMLU/Arena Elo question, +0.500). The net of these four fully accounts for the aggregate drop; there is no unexplained residual.

**Interpretation:** consistent with judge non-determinism on borderline claims, not a pipeline regression. Faithfulness scoring decomposes answers into atomic claims and asks the local judge (`gemma4:26b`, temperature=0) to verify each against retrieved context — on claims that are partially supported or inference-dependent rather than directly quoted, a local model's verdict can flip run to run even at temp=0, since Ollama's inference isn't guaranteed bit-identical across runs (batching/threading effects). A real regression would be expected to move consistently in one direction across affected questions; this diff shows both directions on a small, borderline subset, which is the signature of judge noise rather than decay.

**Action taken:** none — no code, prompt, or pipeline change made. Logged as a monitoring note, not reopened as a Phase 2 defect. **To revisit:** if faithfulness on these same 3–4 questions keeps dropping in the same direction on a further rerun, escalate from "noise" to "real weak spot" and investigate those specific claims. General judge-variance reduction (larger/different judge model, multi-pass averaging) is legitimate Phase 3/4 observability scope, not Phase 2 rework.
